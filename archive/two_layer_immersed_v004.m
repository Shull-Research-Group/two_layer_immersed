function varargout = two_layer_immersed_v004(varargin)
% Shull research group. Original author: Chyi-Huey Joshua Yeh
%This version is currently in beta mode
% TWO_LAYER_IMMERSED_V004 MATLAB code for two_layer_immersed_v004.fig
%      TWO_LAYER_IMMERSED_V004, by itself, creates a new TWO_LAYER_IMMERSED_V004 or raises the existing
%      singleton*.
%
%      H = TWO_LAYER_IMMERSED_V004 returns the handle to a new TWO_LAYER_IMMERSED_V004 or the handle to
%      the existing singleton*.
%
%      TWO_LAYE R_IMMERSED_V004('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TWO_LAYER_IMMERSED_V004.M with the given input arguments.
%
%      TWO_LAYER_IMMERSED_V004('Property','Value',...) creates a new TWO_LAYER_IMMERSED_V004 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before two_layer_immersed_v004_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to two_layer_immersed_v004_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help two_layer_immersed_v004

% Last Modified by GUIDE v2.5 22-Nov-2016 16:18:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @two_layer_immersed_v004_OpeningFcn, ...
                   'gui_OutputFcn',  @two_layer_immersed_v004_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before two_layer_immersed_v004 is made visible.
function two_layer_immersed_v004_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to two_layer_immersed_v004 (see VARARGIN)

% Choose default command line output for two_layer_immersed_v004
...
% UIWAIT makes two_layer_immersed_v004 wait for user response (see UIRESUME)
...
function varargout = two_layer_immersed_v004_OutputFcn(hObject, eventdata, handles) 
...
covariance=calc_ratio_error(handles,n1,n2,n3,del_f_n2,del_f_n1,del_g_n3,del_f_n3);
harm_ratio_error=2.*sqrt(covariance(1,1));
diss_ratio_error=2.*sqrt(covariance(2,2));
...
contourf(d2lam_var,phi_var,harm_ratio_calc,[linspace(-1,1,500)],'edgecolor','none');
...
% cmap=colormap;
% cmap=[ones(1267,3).*cmap(1,:);cmap;ones(416,3).*cmap(end,:)];
% colormap(s1,cmap);
c1=colorbar; set(c1,'fontsize',12);
set(s1,'climmode','manual','ytick',0:10:90);

set(s1,'fontsize',14);
s2=subplot(1,2,2);%plot the dissipation ratio map
contourf(d2lam_var,phi_var,diss_ratio_calc,-logspace(-2,0,res*2),'edgecolor','none');
...
% cmap=colormap(s2,parula);
% cmap=[ones(1200,3).*cmap(1,:);cmap;ones(1600,3).*cmap(end,:)];
% colormap(s2,cmap);
c2=colorbar; set(c2,'fontsize',12);
set(s2,'climmode','manual');
...
%     contour(s1,d2lam_var,phi_var,harm_ratio_calc,[0.911690132574647-harm_ratio_error 0.965343690302451+harm_ratio_error],'edgecolor','k','linestyle','-','showtext','on');
...
%     contour(s1,d2lam_var,phi_var,diss_ratio_calc,-logspace(-4,0,10),'edgecolor','k','linestyle','--','showtext','on');
    hold(s2,'on');
%     contour(s2,d2lam_var,phi_var,harm_ratio_calc,[0.911690132574647-harm_ratio_error 0.965343690302451+harm_ratio_error],'edgecolor','k','linestyle','-','showtext','on');    
...
%     contour(s2,d2lam_var,phi_var,harm_ratio_calc,linspace(0.99,0.01,10),'edgecolor','k','linestyle','-','showtext','on');
...
function grho_var=grho_var_calc(covariance_var,solved,n1,f1)
%This function propagates error based on the covariance matrix of the
%solved variables. Calculated std is std of Grho at harmonic n1.
%This can be accomplished by calculating the Jacobian of the Grho function
%described by d2lam_n1, phi, and drho
d2lam_n1=solved(1);
phi=solved(2);
drho=solved(3);
grho_var=(grho_x1(d2lam_n1,phi,drho,n1,f1).^2).*covariance_var(1,1)+...
    (grho_x2(d2lam_n1,phi,drho,n1,f1).^2).*covariance_var(2,2)+...
    (grho_x3(d2lam_n1,phi,drho,n1,f1).^2).*covariance_var(3,3)+...
    2.*grho_x1(d2lam_n1,phi,drho,n1,f1).*grho_x2(d2lam_n1,phi,drho,n1,f1).*covariance_var(1,2)+...
    2.*grho_x1(d2lam_n1,phi,drho,n1,f1).*grho_x3(d2lam_n1,phi,drho,n1,f1).*covariance_var(1,3)+...
    2.*grho_x2(d2lam_n1,phi,drho,n1,f1).*grho_x3(d2lam_n1,phi,drho,n1,f1).*covariance_var(2,3);

function deriv=grho_x1(d2lam_n1,phi,drho,n1,f1)
%partial derivative of grho with respect to d2lam_n1
deriv=-4.*((drho.*f1.*n1.*cosd(phi./2)).^2).*(1./d2lam_n1).^3;

function deriv=grho_x2(d2lam_n1,phi,drho,n1,f1)
%partial derivative of grho with respect to phi
deriv=-((drho.*f1.*n1./d2lam_n1).^2).*(sind(phi)./2);

function deriv=grho_x3(d2lam_n1,phi,drho,n1,f1)
%partial derivative of grho with respect to drho
deriv=2.*drho.*(f1.*n1.*cosd(phi./2)./d2lam_n1).^2;

function harm_ratio_var=h_ratio_var(s_fn1,s_fn2,cov_fn1_fn2,del_f_n1,del_f_n2,n1,n2)
%this function calculates the variance assosciated with the harmonic ratio.
...
harm_ratio_var=((n2./n1).^2).*(1./(del_f_n2.^2)).*(s_fn1-2.*((del_f_n1./del_f_n2).^2).*cov_fn1_fn2+((del_f_n1./del_f_n2).^2).*s_fn2);

function diss_ratio_var=d_ratio_var(s_gn3,s_fn3,cov_gn3_fn3,del_g_n3,del_f_n3)
%This function calculates the variance associated with the dissipation ratio.
%This error calculation is based on standard propgation of error. See
%DeNolf et al. Langmuir, 2014 for more details on error analysis.
diss_ratio_var=(1./(del_f_n3.^2)).*(((del_g_n3./del_f_n3).^2).*s_fn3-2.*((del_g_n3./del_f_n3).^2).*cov_gn3_fn3+s_gn3);

function drhos_var=drhos_variance(s_fn1,C)
drhos_var=(C.^2).*s_fn1;

function covariance=covariance_rh_rd(n1,n2,del_f_n1,del_f_n2,del_f_n3,del_g_n3,cov_fn1_fn3,cov_fn2_fn3,cov_fn1_gn3,cov_fn2_gn3)
%This function calculates the covariance between the harmonic and
%dissipation ratios.
covariance=(n2/n1).*(1./(del_f_n2.*del_f_n3)).*...
    ((-del_g_n3./del_f_n3).*cov_fn1_fn3+(del_f_n1./del_f_n2).*(del_g_n3./del_f_n3).*cov_fn2_fn3+cov_fn1_gn3-(del_f_n1./del_f_n2).*cov_fn2_gn3);

function covariance=covariance_rh_drhos(n1,n2,s_fn1,del_f_n1,del_f_n2,cov_fn1_fn2,C)
%This function calculates the covariance between the harmonic ratio and the
%Sauerbrey areal mass

covariance=C.*(n2./n1).*((s_fn1./del_f_n1)-(del_f_n1./(del_f_n2.^2)).*cov_fn1_fn2);

function covariance=covariance_rd_drhos(del_f_n3,del_g_n3,cov_fn1_fn3,cov_fn1_gn3,C)
%This function calculates the covariance between the dissipation ratio and
%the %Sauerbrey areal mass
covariance=(C./del_f_n3).*(cov_fn1_fn3.*(-del_g_n3./del_f_n3)+cov_fn1_gn3);

function covariance=calc_ratio_error(handles,n1,n2,n3,del_f_n2,del_f_n1,del_g_n3,del_f_n3)
...
C=-handles.din.constants.zq/(2*n1*handles.din.constants.f1^2);
data=get(handles.error_values,'userdata');
errors=diag(data);
s_fn1=errors(n1);
s_fn2=errors(n2);
...
cov_fn1_fn3=data(n1,n3);
cov_fn2_fn3=data(n2,n3);
cov_fn1_gn3=data(n1,n3+1);
cov_fn2_gn3=data(n2,n3+1);
cov_fn1_fn2=data(n1,n2);
cov_gn3_fn3=data(n3+1,n3);

harm_ratio_var=h_ratio_var(s_fn1,s_fn2,cov_fn1_fn2,del_f_n1,del_f_n2,n1,n2);%variance in the harmonic ratio
diss_ratio_var=d_ratio_var(s_gn3,s_fn3,cov_gn3_fn3,del_g_n3,del_f_n3);%variance in the dissipation ratio
drhos_var=drhos_variance(s_fn1,C);%variance in drhos calcualted from Sauerbrey relation
Sigma_rh_rd=covariance_rh_rd(n1,n2,del_f_n1,del_f_n2,del_f_n3,del_g_n3,cov_fn1_fn3,cov_fn2_fn3,cov_fn1_gn3,cov_fn2_gn3);%covariance of rh and rd
Sigma_rh_drhos=covariance_rh_drhos(n1,n2,s_fn1,del_f_n1,del_f_n2,cov_fn1_fn2,C);%covariance of rh and drhos
Sigma_rd_drhos=covariance_rd_drhos(del_f_n3,del_g_n3,cov_fn1_fn3,cov_fn1_gn3,C);%covariance of rd and drhos
covariance=[harm_ratio_var, Sigma_rh_rd, Sigma_rh_drhos;...
            Sigma_rh_rd, diss_ratio_var, Sigma_rd_drhos;...
            Sigma_rh_drhos, Sigma_rd_drhos, drhos_var];%covariance matrix of the functions
...
for dum0=1:length(radiotot)
   ...
   try
        ...
        harm_ratio_exp=(del_f_n2.*n1)./(del_f_n1.*n2);%experimental harmonic ratio
        diss_ratio_exp=del_g_n3./del_f_n3;%experimental dissipation ratio
        %assemble the covariance matrix of the harm and diss ratio
        covariance_ratios=calc_ratio_error(handles,n1,n2,n3,del_f_n2,del_f_n1,del_g_n3,del_f_n3);%calculate the stdev of the ratios        
    ...
    try%calculate the error
        covariance_var=inv(full(output.jacobian))*covariance_ratios*inv(transpose(full(output.jacobian)));
        disp(' ');
        for dum=1:length(unique_n)       
            %store the drho value lb and ub and timepoint
            name=['n_',num2str(unique_n(dum)),'_',num2str(n1),num2str(n2),num2str(n3)];
            handles.din.stored_solutions.(name).error.drho(count,:)=[handles.din.cursor.selected_time,sqrt(covariance_var(3,3)).*1000];%in units of g/m^2
            %store the grho value lb and ub and timepoint
            handles.din.stored_solutions.(name).error.grho(count,:)=[handles.din.cursor.selected_time,sqrt(grho_var_calc(covariance_var,solved,unique_n(dum),f1))./1000];%in units of Pa-g/cc
            %store the phi value lb and ub and timepoint
            handles.din.stored_solutions.(name).error.phi(count,:)=[handles.din.cursor.selected_time,sqrt(covariance_var(2,2))];%in degrees
            %store the d2lamn value lb and ub and timepoint
            handles.din.stored_solutions.(name).error.d2lam(count,:)=[handles.din.cursor.selected_time,sqrt(covariance_var(1,1))];
        %store the complex pred. freq. values lb and ub and timepoint
%         handles.din.stored_solutions.(name).error.complex_pred_freq(count,:)=[handles.din.cursor.selected_time,];         error was not calculated for the predicted shifts
        end
    catch err_msg
        assignin('base','err_msg',err_msg);
