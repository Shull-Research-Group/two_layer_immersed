...
%
%      TWO_LAYER_IMMERSED_V004('CALLBACK',hObject,eventData,handles,...) calls the local
...
contourf(d2lam_var,phi_var,real(qcm_map),linspace(-2,1,300),'edgecolor','none');hold on;
...
contourf(d2lam_var,phi_var,imag(qcm_map),linspace(0,1,300),'edgecolor','none');hold on;
...
function grho_var=grho_std(covariance_var,solved,n1,f1)
...
grho_var=(grho_x1(d2lam_n1,phi,drho,n1,f1).^2).*covariance_var(1,1)+...
    (grho_x2(d2lam_n1,phi,drho,n1,f1)).*covariance_var(2,2)+...
    (grho_x3(d2lam_n1,phi,drho,n1,f1)).*covariance_var(3,3)+...
...
covariance=C.*(n1./n2).*((s_fn1./del_f_n1)-(del_f_n1./(del_f_n2.^2)).*cov_fn1_fn2);
...
for dum0=1:length(radiotot)
   ...
   try
    ...
    try%calculate the error
        covariance_var=inv(output.jacobian)*covariance_ratios*inv(transpose(output.jacobian));
        disp(' ');
        for dum=1:length(unique_n)       
            ...
            handles.din.stored_solutions.(name).error.grho(count,:)=[handles.din.cursor.selected_time,sqrt(grho_std(covariance_var,solved,unique_n(dum),f1))./1000];%in units of Pa-g/cc
